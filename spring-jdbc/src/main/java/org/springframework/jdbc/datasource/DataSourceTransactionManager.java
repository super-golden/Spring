...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	...
	/**
	 * 这里是产生Transaction的地方，为Transaction 的创建提供服务
	 * 1、对数据库而言，事务工作是由Connection来完成的。这里把数据库的Connection 对象放到一个
	 *   ConnectionHolder中，然后封到一个 DataSourceTransactionObject 对象中，在这个封装过程
	 *   中增加了许多为事务处理服务的控制数据
	 *
	 */
	@Override
	protected Object doGetTransaction() {
		...
		/*获取与当前线程绑定的数据库Connection，这个Connection 在第一个事务开始的地方与线程绑定*/
		ConnectionHolder conHolder =
				(ConnectionHolder) TransactionSynchronizationManager.getResource(obtainDataSource());
		txObject.setConnectionHolder(conHolder, false);
		return txObject;
	}
    //这里判断是否已经存在事务的地方，由ConnectionHolder 的 isTransactionActive属性来控制
	@Override
	...
	//事务回滚过程，使用Connection 的rollback方法
	@Override
