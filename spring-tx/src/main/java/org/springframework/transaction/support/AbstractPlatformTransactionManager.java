...
/**
 * 事务处理器基类（提供了很多模版方法）
 *     AbstractPlatformTransactionManager封装了Spring 事务处理中通用的处理部分，比如事务的创建、提交、回滚、事务状态和信息处理
 *     ，与线程的绑定等。有了这些通用处理的支持，对于具体的事务管理器而言，他们只需要处理和具体数据源相关的组件设置就可以了，比如
 *     在HibernateTransactionManager中，就只需要配置好和Hibernate事务处理相关的接口以及相关的设置。
 */
...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
	   ...
	   */
		...
		/**
		 * 这个doGetTransaction() 是抽象函数，Transaction 对象的取得由具体的事务处理器实现，比如 DataSourceTransactionManager
		 */
		 ...
		 *
		 * isExistingTransaction() 具体事务处理器实现逻辑不一样，在这个
		 * AbstractPlatformTransactionManager 这个类中只是提供一个模版
		 */
		...
		/**
		 * 当前没有事务存在，这时需要根据事务属性设置来创建事务，这里会看到对事务传播属性设置的处理、比如
		 * mandatory、required、required_new 、nested等。
		 */
	...
	private TransactionStatus startTransaction(TransactionDefinition definition, Object transaction,
		...
		/**
		 * TransactionStatus封装事务执行情况，默认
		 * transactionSynchronization = SYNCHRONIZATION_ALWAYS;
		 * 所以 newSynchronization 为true
		 */
		DefaultTransactionStatus status = newTransactionStatus(
				definition, transaction, true, newSynchronization, debugEnabled, suspendedResources);
		/**
		 * 这里是创建事务的调用，由具体的事务处理器来完成，比如HibernateTransactionManager和
		 * DataSourceTransactionManager等
		 */
	 ...
	 * 当前线程中存在事务的情况下创建TransactionStatus 对象
	 */
	private TransactionStatus handleExistingTransaction(
			TransactionDefinition definition, Object transaction, boolean debugEnabled)
			throws TransactionException {
		/*如果当前线程已有事务存在，且当前事务的传播属性设置是never,那么抛出异常，说明这种情况是有问题的
		 * spring 无法处理当前的事务创建
		 */
		...
		/* 如果当前事务的配置属性是PROPAGATION_NOT_SUPPORTED ，同时当前线程已经存在事务了
		 * 那么将事务挂起
		 */
        ...
        /*
          ...
          与创建全新事务的过程类似，区别在于，在创建全新事务时不用考虑已有事务的挂起，但这里需要考虑已有事务的挂起处理
         */
	...
	protected DefaultTransactionStatus newTransactionStatus(
		 ...
		 * TransactionSynchronizationManager 维护一系列的ThreadLocal变量来保持事务属性
	 ...
	 * @see #resume
	 */
	//返回的SuspendedResourcesHolder 会作为参数传递给TransactionStatus
	@Nullable
	protected final SuspendedResourcesHolder suspend(@Nullable Object transaction) throws TransactionException {
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			...
			try {
				...
				//这里在线程中保存与事务处理有关的信息，并重置线程相关的ThreadLocal变量
	 ...
	 * @see #rollback
	 */
	//AbstractPlatformTransactionManager 中的模版方法，支持具体的事务处理器对事务提交的实现。
	@Override
	public final void commit(TransactionStatus status) throws TransactionException {
		//在TransactionStatus 中标识事务已经结束
	...
	private void processCommit(DefaultTransactionStatus status) throws TransactionException {
		try {
			...
			try {
				...
				/*
				 下面对根据当前线程中保存的事务状态进行处理，如果当前的事务是一个新事务，调用具体的事务处理器的完成提交。
	...
	private void processRollback(DefaultTransactionStatus status, boolean unexpected) {
		try {
			...
			try {
				...
				else if (status.isNewTransaction()) {//当前事务调用方法中新建事务的回滚处理
					if (status.isDebug()) {
						logger.debug("Initiating transaction rollback");
					}
					//这个doRollback()处理是由具体的事务处理器来完成的
					doRollback(status);
				}
				else {//如果当前事务调用方法中没有新建事务的回滚处理
