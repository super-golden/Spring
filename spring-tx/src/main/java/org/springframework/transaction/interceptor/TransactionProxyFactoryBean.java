...
public class TransactionProxyFactoryBean extends AbstractSingletonProxyFactoryBean
	...
	/**
	 * 这个拦截器TransactionInterceptor 通过AOP发挥作用，通过这个拦截器的实现，Spring 封装了事务处理实现，关于它的具体实现，下面会详细地分析
	 */
	 ...
	 */
	//通过setter依赖注入的 PlatformTransactionManager(可以理解成这个类中有个transactionManager属性)
	 ...
	 */
	/**
	 * 通过依赖注入的事务属性以Properties 的形式出现
	 * 把BeanDefinition 中读取的事务管理的属性信息注入到TransactionInterceptor 中
	...
	protected Object createMainInterceptor() {
		...
		if (this.pointcut != null) {
			//这里使用默认的通知器DefaultPointcutAdvisor ,事务处理拦截器
			return new DefaultPointcutAdvisor(this.pointcut, this.transactionInterceptor);
		}
		else {
			/**
			 * 如果没有pointcut ,使用TransactionAttributeSourceAdvisor 作为通知器，并为通知器设置transactionInterceptor 作为拦截器
			 */
