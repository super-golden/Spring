...
/*
XML配置文件的读取流程：
1、通过继承自AbstractBeanDefinitionReader 中的方法，来使用ResourceLoader将资源文件路径转换为对应的Resoure文件。
2、通过DocumentLoader对Resource文件进行转换，将Resource文件转换为Document文件。
3、通过实现接口 BeanDefinitionDocumentReader 的DefaultBeanDefinitionDocumentReader类对Document进行解析，并使用
BeanDefinitionParserDelegate 对Element进行解析。
 */
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	 ...
	 * in order to still processores schema namespaces properly in such a scenario.
	 *设置是否使用XML验证。默认值是{@code true}。
	 *<p>如果关闭验证，此方法将打开命名空间感知，
	 ...
	 * in order to properly processores schema namespaces.
	 */
	 ...
	 */
	/**
	 * 如果SAX应用程序需要实现自定义处理外部实体，则必须实现此接口并使用setEntityResolver方法向SAX
	 * 驱动器注册一个实例。也就是说，对于解析一个XML，SAX首先读取XML文档上的声明，根据声明去寻找相应的
	 * DTD定义，以便对文档进行一个验证。
	 * 默认的寻找规则，即通过网络（实际上就是声明的DTD的URI地址）来下载相应的DTD声明，并进行认证。
	 *
	 *EntityResolver的作用是项目本身就可以提供一个如何寻找DTD声明的方法，即由程序来实现寻找DTD声明的过程。
	 * 比如我们将DTD文件放到项目中某处，在实现时直接将此文档读取并返回给SAX即可。
	 * 这样避免了通过网络来寻找相应的声明。
	 * @return
	 */
	 ...
	 */
	/*
	在读取器中，需要得到代表XML文件的Resource，因为这个Resource对象封装了对XML文件 的I/O操作，所以读取器可以在打开
	I/O流后得到XML的文件对象。有了这个对象以后，就可以按照Spring的Bean定义规则来对这个 XML文档树进行解析了，
	这个解析是交给BeanDefinitionParserDelegate来完成的
	 */
	@Override
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		//（1）封装资源文件。使用EncodedResource封装Resource
	 ...
	 *允许指定用于分析文件的编码
	 *@返回找到的bean定义数
	 *@throws BeanDefinitionStoreException 以防加载或分析错误
	 */
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try (InputStream inputStream = encodedResource.getResource().getInputStream()) {
			//InputSource 这个类不是来源于Spring，而是org.xml.sax
 ...
 */
	 ...
	 *<p>创建parser类的新实例并调用
	 *{@code registerBeanDefinitions}在上面。
	 *@param doc DOM文档
	 *@param resource 资源描述符（用于上下文信息）
	 *@返回找到的bean定义数
	 *@throws BeanDefinitionStoreException 以防分析错误
	 *@请参阅loadBeanDefinitions
	 *@请参阅setDocumentReaderClass
	 *@请参阅BeanDefinitionDocumentReader#registerBeanDefinitions
	 */
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		//这里得到 BeanDefinitionDocumentReader（DefaultBeanDefinitionDocumentReader） 来对XML的BeanDefinition 进行解析
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();

		//记录统计前BeanDefinition的加载个数
		int countBefore = getRegistry().getBeanDefinitionCount();
		//具体的解析过程在这个registerBeanDefinitions 中完成
