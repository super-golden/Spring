 ...
 */
/*
BeanDefinition 在Spring中存在三种实现：RootBeanDefinition、ChildBeanDefinition 以及GenericBeanDefinition。三种实现均继承了AbstractBeanDefinition。
在配置文件中可以定义父<bean>和子<bean>,父<bean>用RootBeanDefinition表示，而子<bean>用ChildBeanDefinition表示，而没有父<bean>的就用RootBeanDefinition表示。
AbstractBeanDefinition对两者共同的类信息进行抽象。
Spring 通过BeanDefinition将配置文件中的<bean>配置信息转换为容器的内部表示，并将这些BeanDefinition注册到BeanDefinitionRegistry
中。
 */
...
//返回/设置 当前的BeanClassName（不等于最终Bean的名称）
	 ...
	 * 如果其他对象按照类型自动装配时发现有多个符合类型的多个实现bean，
	 * 如果bean的primary属性为true，
...
//返回设置 factory bean name
	 ...
	 * 设置bean工厂的方法名，Spring在实例化Bean对象时支持工厂方法设计模式，
	 * 在初始化bean时不是通过bean的class发射创建 bean实例，
	 * 而是根据factoryBeanName反射出工厂的实例，
	 * 然后调用它的实例方法factoryMethodName来创建bean实例。
	 */
