...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * autowire-candidate 属性设置为false，这样容器在查找自动装配对象时，将不考虑该bean，即它不会被考虑
	 * 作为其他bean自动装配的候选者，但是该bean本身还是可以使用自动装配来注入其他bean的
	...
	/**
	 * Return a callback for creating an instance of the bean, if any.返回用于创建bean实例的回调（如果有）。
	 * @since 5.0
	 */
	@Nullable
	...
	/**
	 * Return the name of the initializer method.返回初始值设定项方法的名称。
	 */
	@Override
	@Nullable
	...
	/**
	 * Return whether this bean definition is 'synthetic', that is,返回此bean定义是否为“synthetic”，即，
	 * not defined by the application itself. 不是由应用程序本身定义的。
	 */
	...
	/*
	对override属性进行标记及验证。
	    在spring的配置里面根本就没有诸如override-method之类的配置，那么这个方法到底是干什么用的呢？
	       其实在Spring 中确实没有override-method这样的配置，但是在Spring配置中是存在 lookup-method 和 replace-method
	       的，而这两个配置的加载其实就是将配置统一放在BeanDefinition中的methodOverrides属性里，而这个函数的操作其实也就是针对于这两个
	       配置的。
	 */
