 ...
 * {@linkplain #processContextConfiguration processores} the
...
public abstract class AbstractDelegatingSmartContextLoader implements SmartContextLoader {
	...
	/**
	 * Delegates to candidate {@code SmartContextLoaders} to processores the supplied
	 ...
	 * candidate loader will be allowed to processores the configuration <em>as is</em>,
	 * without any checks for detection of defaults.</li>
	 * <li>Otherwise, the XML-based loader will be allowed to processores
	 ...
	 * processores the configuration in order to detect default configuration classes.
	 * If the annotation-based loader detects default configuration
	 * classes, an {@code info} message will be logged.</li>
	 * </ul>
	 * @param configAttributes the context configuration attributes to processores
	...
	public void processContextConfiguration(final ContextConfigurationAttributes configAttributes) {
		...
		Assert.isTrue(!(configAttributes.hasLocations() && configAttributes.hasClasses()),
				() -> String.format("Cannot processores locations AND classes for context configuration %s: " +
		...
		// appropriate loader processores the configuration.
		if (configAttributes.hasLocations()) {
			...
			// Let the XML loader processores the configuration.
			delegateProcessing(getXmlLoader(), configAttributes);
			...
			// Now let the annotation config loader processores the configuration.
			delegateProcessing(getAnnotationConfigLoader(), configAttributes);
	...
	private static void delegateProcessing(SmartContextLoader loader, ContextConfigurationAttributes configAttributes) {
		if (logger.isDebugEnabled()) {
			logger.debug(String.format("Delegating to %s to processores context configuration %s.",
