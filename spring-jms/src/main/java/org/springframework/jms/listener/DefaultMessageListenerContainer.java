...
public class DefaultMessageListenerContainer extends AbstractPollingMessageListenerContainer {
	...
	/**
	 * Specify the maximum number of messages to processores in one task.
	...
	/**
	 * Return the maximum number of messages to processores in one task.
	 */
	...
	public void stop(Runnable callback) throws JmsException {
		synchronized (this.lifecycleMonitor) {
			if (!isRunning() || this.stopCallback != null) {
				...
				// -> return immediately, no stop processores to control anymore.
				callback.run();
				return;
			}
