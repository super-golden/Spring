...
public abstract class AbstractAdvisorAutoProxyCreator extends AbstractAutoProxyCreator {
	...
	/**
	 * 对于指定bean的增强方法的获取一般是包含两个步骤的
	 * 1、获取所有的增强（findCandidateAdvisors()）
	 * 2、寻找所有增强中适用于bean的增强并应用（findAdvisorsThatCanApply(candidateAdvisors, beanClass, beanName)）。
	 * 如果无法找到对应的增强便会返回DO_NOT_PROXY（null）
	...
	protected List<Advisor> findEligibleAdvisors(Class<?> beanClass, String beanName) {
		//找到ioc容器中的候选通知（获取所有的增强）
		List<Advisor> candidateAdvisors = findCandidateAdvisors();
		//判断我们的通知能不能作用到当前的类上（寻找所有增强中适用于bean的增强并应用）
	 ...
	 *  前面的函数中已经完成了所有增强器的解析，但对于所有的增强器来讲，并不一定都适用于当前的bean，还要找出合适的增强器
	 *  也就是满足我们配置的通配符的增强器
	 */
