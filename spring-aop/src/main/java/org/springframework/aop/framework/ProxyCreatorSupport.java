...
public class ProxyCreatorSupport extends AdvisedSupport {
	...
	protected final synchronized AopProxy createAopProxy() {
		if (!this.active) {
			activate();
		}//这里使用DefaultAopProxyFactory 来创建AopProxy
		/**
		 * 因为这个ProxyFactory类本身就是ProxyConfig的子类，所以这里创建AopProxy的过程和一般的Proxy代理的创建过程是一样的
		 */
		//通过AopProxyFactory 获取AopProxy，这个AopProxyFactory 是在这个ProxyCreatorSupport中定义的
		//	public ProxyCreatorSupport() {
		//		this.aopProxyFactory = new DefaultAopProxyFactory();
		//	}
