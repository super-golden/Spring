...
/**
 *  ProxyFactoryBean  是一个FactoryBean
...
public class ProxyFactoryBean extends ProxyCreatorSupport
	 ...
	 */
	/**
	 * 对ProxyFactoryBean 来说，把需要对target 目标对象增加的增强处理，都通过getObject方法进行了封装，这些增强处理是为了AOP
	 * 功能的实现提供服务的。
	 * @return
	...
	public Object getObject() throws BeansException {
		//对通知器链进行初始化，通知器链封装了一系列的拦截器，这些拦截器都要从配置中读取，然后为代理对象的生成做好准备。
	 ...
	 */
	//通过createAopProxy 返回的AopProxy 来得到代理对象
			...
			for (String name : this.interceptorNames) {
				if (name.endsWith(GLOBAL_SUFFIX)) {
					...
					if (this.singleton || this.beanFactory.isSingleton(name)) {
						...
						//这里是取得advisor 的地方，是通过beanFactory  获取的、把interceptorNames这个List中的interceptor名字交给
