...
final class JdkDynamicAopProxy implements AopProxy, InvocationHandler, Serializable {
	 ...
	 *查找任何可以定义的{@link#equals}或{@link#hashCode}方法
	 *在提供的接口集上。
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		try {
			 ...
			 */
				...
				/**
				 * 如果没有设置拦截器，那么会对目标对象的方法直接进行调用。对于JdkDynamicAopProxy代理对象，这个对目标
				 * 对象的方法调用是通过AopUtils使用反射机制在AopUtils.invokeJoinpointUsingReflection方法中实现的。
				 */
			...
			//得到目标对象的地方
			//通过getTarget方法，完成了HotSwappableTargetSource 与AOP的集成。这个热交换功能为AOP的使用提供了更多便利。
			...
			if (chain.isEmpty()) {
				...
				//注意，最后的调用程序必须是InvokerInterceptor，所以我们知道它是
				//只有对目标的反射操作，没有热交换或花式代理。
				Object[] argsToUse = AopProxyUtils.adaptArgumentsIfNecessary(method, args);
				retVal = AopUtils.invokeJoinpointUsingReflection(target, method, argsToUse);
			}
			else {
				// We need to create a method invocation...
				//如果有拦截器的设定，那么需要调用拦截器之后菜调用目标的对应方法
