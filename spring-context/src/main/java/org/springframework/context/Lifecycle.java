...
/**
 * Spring中还提供了Lifecycle接口，Lifecycle中包含start/stop方法，实现此接口后Spring会保证在启动的时候调用器start方法开启生命周期，
 * 并在Spring 关闭的时候调用stop方法来结束生命周期，通常用来配置后台程序，在启动后一直运行（如对MQ进行轮询等）。而在ApplicationContext的
 * 初始化最后正是保证了这一功能的实现。
 */
