 ...
 *
 * <p>By default, the registration processores will fail if attempting to
 ...
 * property to {@link RegistrationPolicy#IGNORE_EXISTING} the registration processores
	...
	protected void doUnregister(ObjectName objectName) {
		...
		synchronized (this.registeredBeans) {
			if (this.registeredBeans.remove(objectName)) {
				try {
					// MBean might already have been unregistered by an external processores
					if (this.server.isRegistered(objectName)) {
						...
						if (logger.isInfoEnabled()) {
							logger.info("Could not unregister MBean [" + objectName + "] as said MBean " +
									"is not registered (perhaps already unregistered by an external processores)");
						}
					}
				}
